 ...
 * 
 *  @(#) $Id: resolver.cpp 1615 2018-08-02 11:59:17Z coas-nagasima $
 */
...
static char	dns_domain_name[DNS_NAME_LENGTH + 1] = RSLV_CFG_DNS_DOMAIN_NAME_STR;
...
add_name (uint8_t *msg, uint_t size, uint_t offset, char *name)
{
	...
	while (*name != '\0') {
		...
		/* ラベル長を設定する。*/
		label_len = (uint_t)tail - (uint_t)name;
...
dns_analyze_qd (T_RSLV_QD *qd, uint_t offset, uint8_t *msg, uint_t length)
{
	...
	qd->dnscls = MSG2HOST16(dns_qd->dnscls);
...
dns_analyze_rr (T_RSLV_RR *rr, uint_t offset, uint8_t *msg, uint_t length)
{
	...
	rr->dnscls	= MSG2HOST16(dns_rr->dnscls);
	rr->ttl		= MSG2HOST32(dns_rr->ttl);
	rr->rdlength	= MSG2HOST16(dns_rr->rdlength);
	rr->rdata_offset= offset + sizeof(rr->type) + sizeof(rr->dnscls)
 ...
 *
 *    注意: name に NULL を指定すると、初期値に戻す。
 */

const char *
dns_in6_set_dname (const char *name, uint_t len)
{
	char	*dst;
...
T_IN6_ADDR *
dns_in6_set_addr (T_IN6_ADDR *src)
{
	if (src == NULL)
...
const char *
dns_in4_set_dname (const char *name, uint_t len)
{
	char	*dst;
...
T_IN4_ADDR *
dns_in4_set_addr (T_IN4_ADDR *src)
{
	if (src == NULL)
...
dns_host_addr (uint16_t flags, char *hostname, T_IN_ADDR *addr)
{
	...
	for (scount = 1; scount <= rslv.dns_hdr.ancount; scount ++) {
		...
		if (rr.type == DNS_TYPE_AAAA && rr.dnscls == DNS_CLASS_IN &&  rr.rdlength == sizeof(*addr)) {
		...
		if (rr.type == DNS_TYPE_A && rr.dnscls == DNS_CLASS_IN && rr.rdlength == sizeof(addr4)) {
		...
		if (rr.type == DNS_TYPE_A && rr.dnscls == DNS_CLASS_IN && rr.rdlength == sizeof(*addr)) {
