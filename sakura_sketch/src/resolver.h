 ...
 * 
 *  @(#) $Id: resolver.h 1614 2018-08-01 23:13:43Z coas-nagasima $
 */
...
ER_UINT	dns_lookup_host (uint16_t flags, char *hostname, uint8_t *msg, uint_t msg_len, T_RSLV_DNS_MSG *rslv);
ER	dns_host_addr (uint16_t flags, char *hostname, T_IN_ADDR *addr);
ER_UINT	dns_analyze_qd (T_RSLV_QD *qd, uint_t offset, uint8_t *msg, uint_t length);
ER_UINT	dns_analyze_rr (T_RSLV_RR *rr, uint_t offset, uint8_t *msg, uint_t length);
ER_UINT	dns_analyze_soa (T_RSLV_SOA *soa, uint_t offset, uint8_t *msg, uint_t length);
...
const char *dns_in6_set_dname (const char *name, uint_t len);
const char *dns_in6_get_dname (void);
const char *dns_in4_set_dname (const char *name, uint_t len);
const char *dns_in4_get_dname (void);
T_IN6_ADDR *dns_in6_set_addr (T_IN6_ADDR *src);
T_IN6_ADDR *dns_in6_get_addr (T_IN6_ADDR *dst);
T_IN4_ADDR *dns_in4_set_addr (T_IN4_ADDR *src);
T_IN4_ADDR *dns_in4_get_addr (T_IN4_ADDR *dst);
