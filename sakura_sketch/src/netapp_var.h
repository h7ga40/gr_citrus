 ...
 * 
 *  @(#) $Id: netapp_var.h 1615 2018-08-02 11:59:17Z coas-nagasima $
 */
...
const char *in_strtfn (FN fncd);
const char *dns_strtype (uint_t type);
const char *dns_strclass (uint_t dnscls);
ER	net_syslog(uint_t prio, const char *format, ...) throw();
ER	net_serial_ctl_por(uint_t ioctl) throw();
void	flush_snd_buff (void);
ER	discon_net_cons (void);
void	wup_net_cons (void);
bool_t	tcp6_is_cepid (ID cepid);
ER	alloc_tcp6_rep (ID *repid, ID tskid, T_TCP6_CREP *crep);
ER	alloc_tcp6_cep (ID *cepid, ID tskid, T_TCP_CCEP *ccep);
ER	alloc_udp6_cep (ID *cepid, ID tskid, T_UDP6_CCEP *ccep);
ER	free_tcp6_rep (ID repid, bool_t call_tcp_del_rep);
ER	free_tcp6_cep (ID cepid);
ER	free_udp6_cep (ID cepid, bool_t call_udp_del_cep);
bool_t	tcp4_is_cepid (ID cepid);
ER	alloc_tcp4_rep (ID *repid, ID tskid, T_TCP_CREP *crep);
ER	alloc_tcp4_cep (ID *cepid, ID tskid, T_TCP_CCEP *ccep);
ER	alloc_udp4_cep (ID *cepid, ID tskid, T_UDP_CCEP *ccep);
ER	free_tcp4_rep (ID repid, bool_t call_tcp_del_rep);
ER	free_tcp4_cep (ID cepid);
ER	free_udp4_cep (ID cepid, bool_t call_udp_del_cep);
void	ping6 (T_IN6_ADDR *addr, uint_t tmo, uint_t len);
void	ping4 (T_IN4_ADDR *addr, uint_t tmo, uint_t len);
const char	*get_ipv6addr (T_IN6_ADDR *addr, const char *line);
const char	*get_ipv4addr (T_IN4_ADDR *addr, const char *line);
const char	*resolv_options (uint_t *flags, const char *line, char apip);
const char	*resolv_hoststr (uint_t *flags, char *hostname, uint_t name_size, const char *line);
const char	*lookup_ipaddr (T_IN_ADDR *addr, const char *line, char *apip);
uint_t	show_dns_domain_name (uint8_t *hdr, uint_t offset);
void	host2msg16 (void *msg, uint16_t host_data);
void	host2msg32 (void *msg, uint32_t host_data);
uint16_t	msg2host16 (void *msg);
uint32_t	msg2host32 (void *msg);
