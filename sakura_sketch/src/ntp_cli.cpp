 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: ntp_cli.cpp 1615 2018-08-02 11:59:17Z coas-nagasima $
 */
...
void ntp_cli_timeout(T_NTP_CLI_CONTEXT *nc)
{
	...
	case NTP_CLI_STATE_ASYNC:
		...
		apip = API_PROTO_IPV4;
		line = lookup_ipaddr(&nc->ipaddr6, NTP_SRV_URL, &apip);
		...
		if ((line = lookup_ipaddr(&nc->snd_rmt.ipaddr, NTP_SRV_URL, &apip)) == NULL) {
			syslog(LOG_NOTICE, "[NTP CLI,%d] sleep %d.%03u[s], unknown host.",
				nc->cepid, SLP_ITV / SYSTIM_HZ, SLP_ITV % SYSTIM_HZ);
			nc->timer = SLP_ITV;
			break;
