 ...
 * 
 *  @(#) $Id: netapp_subr.cpp 1615 2018-08-02 11:59:17Z coas-nagasima $
 */
...
cons_getchar ()
{
	...
	if (Serial.readBytes(&ch, sizeof(ch)) > 0)
		return ch;
	else
		return EOF;
	}
...
int_t
cons_putnumber(ulong_t 	val, int_t radix,
         const char *radchar, int_t width, bool_t minus, char padchar)
{
	...
	if (!left)	/* 右詰め */
		for ( ; pad < width; pad ++)
			cons_putchar(padchar);

	while (ix -- > 0)
		cons_putchar(digits[ix]);
...
int_t
put_ipv4addr (T_IN4_ADDR *addr, int_t width)
{
	int_t len = 3;	/* 3 は '.' の文字数 */

	len += cons_putnumber((*addr >> 24) & 0xff, 10, radhex, 0, false, ' '); 
	cons_putchar('.');
	len += cons_putnumber((*addr >> 16) & 0xff, 10, radhex, 0, false, ' '); 
	cons_putchar('.');
	len += cons_putnumber((*addr >>  8) & 0xff, 10, radhex, 0, false, ' '); 
	cons_putchar('.');
	len += cons_putnumber( *addr        & 0xff, 10, radhex, 0, false, ' '); 
...
int_t
put_ipv6addr (const T_IN6_ADDR *addr, int_t width)
{
	int_t	len = 0, ix, len6;
	...
	if (addr == NULL || IN6_IS_ADDR_UNSPECIFIED(addr)) {
		...
		for (ix = 0; ix < len6; ix ++) {
			if (omit) {
				len += cons_putnumber(ntohs(addr->s6_addr16[ix]), 16, radhex, 0, false, ' '); 
				if (ix < 7) {
					cons_putchar(':');
					len ++;
					}
...
int_t
put_macaddr (uint8_t *mac, int_t width)
{
	int_t oct, len;

	for (oct = 5; oct -- > 0; ) {
		cons_putnumber(*mac ++, 16, radhex, 2, false, '0'); 
		cons_putchar(':');
		}
	cons_putnumber(*mac, 16, radhex, 2, false, '0'); 
...
cons_printf (const char *fmt, ...)
{
	...
	while ((ch = *fmt ++) != '\0') {
		...
		case 'd':
			...
			if (val >= 0)
				cons_putnumber( val, 10, radhex, width * left, false, padchar);
			else
				cons_putnumber(-val, 10, radhex, width * left, true, padchar);
			break;
		...
		case 'x':
			val = longflag ? (ulong_t)va_arg(ap, ulong_t)
			               : (ulong_t)va_arg(ap, uint_t);
			cons_putnumber(val, 16, radhex, width * left, false, padchar);
			break;
		...
		case 'I':
			...
			if (longflag) {
				str = va_arg(ap, char*);
				put_ipv6addr((T_IN6_ADDR *)str, width);
				}
			else if (shortflag) {
				addr = va_arg(ap, T_IN4_ADDR *);
				put_ipv4addr(addr, width);
				}
			else {
...
const char *
get_ipv4addr (T_IN4_ADDR *addr, const char *line)
{
	int_t oct;
...
const char *
get_ipv6addr (T_IN6_ADDR *addr, const char *line)
{
...
int_t
cons_getline (char *line, int_t size)
{
	int_t	ch, len;

	len = size;
	while (size > 0) {
		if ((ch = cons_getchar()) != EOF) {
			if (ch == '\r') {
				cons_putchar('\n');
				break;
				}
...
const char *
get_int (int_t *val, const char *line)
{
	int_t sign = 1;
...
const char *
get_xuint (uint_t *val, const char *line)
{
	line = skip_blanks(line);
	*val = 0;
	while (1) {
...
const char *
resolv_hoststr (uint_t *flags, char *hostname, uint_t name_size, const char *line)
{
	char *h;
	const char *p;
...
uint_t
show_dns_domain_name (uint8_t *hdr, uint_t offset)
{
	uint8_t	*ptr;
	uint_t	c;
	...
	while (*ptr) {
		if ((*ptr & DNS_MSG_COMP_MARK) == DNS_MSG_COMP_MARK) {
			show_dns_domain_name(hdr, (*ptr & ~DNS_MSG_COMP_MARK) << 8 | *(ptr + 1));
			ptr += 2;
			break;
			}
		else {
			for (c = 1; c <= *ptr; c++)
				syslog(LOG_NOTICE, "%c", *(ptr + c));
			ptr += *ptr + 1;
			if (*ptr)
				syslog(LOG_NOTICE, ".");
			}
...
const char *
resolv_options (uint_t *flags, const char *line, char apip)
{
#if defined(SUPPORT_INET6)
	T_IN6_ADDR	in6_addr;
#endif
...
const char *
lookup_ipaddr (T_IN_ADDR *addr, const char *line, char *apip)
{
	...
	*apip = 0;
	...
	if ((last = resolv_hoststr(&flags, hostname, sizeof(hostname), line)) == NULL) {
		syslog(LOG_NOTICE, "unknown host: %s.\n", hostname);
		return NULL;
		}
	...
	case DNS_LUP_FLAGS_NAME_FQDN:
		...
		if ((flags & (DNS_LUP_FLAGS_PROTO_IPV6 | DNS_LUP_FLAGS_PROTO_IPV4)) == 0) {
			syslog(LOG_NOTICE, "DNS server not available.\n");
			return NULL;
			}
	...
	if ((flags & DNS_LUP_FLAGS_PROTO_IPV6) != 0)
		*apip = API_PROTO_IPV6;
	else
		*apip = API_PROTO_IPV4;
