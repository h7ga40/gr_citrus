 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: prc_config.c 1605 2018-07-29 15:33:03Z coas-nagasima $
 */
...
x_config_int( INTNO intno, ATR intatr, PRI intpri )
{
	...
	if( INTNO_IRQ( intno ) != 0U ){
		...
		sil_wrb_mem((uint8_t *)irqcr_reg_addr[irq_offset], irq_val);
	}
	 ...
	 *  割込み番号に対応したIPR値を設定する.
	 */
	sil_wrb_mem((uint8_t *)ipr_reg_addr[intno], cfg_int_table[intno].intpri);
