 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: cyclic.c 1605 2018-07-29 15:33:03Z coas-nagasima $
 */
...
{
	p_cyccb->tmevtb.callback = (CBACK) call_cychdr;
	p_cyccb->tmevtb.arg = (void *) p_cyccb;
	p_cyccb->tmevtb.evttim = evttim;
	p_cyccb->cycsta = true;
...
sta_cyc(ID cycid)
{
	...
	if (p_cyccb->p_cycinib->cycatr == TA_NOEXS) {
		...
		tmevtb_enqueue_cyc(p_cyccb, p_cyccb->p_cycinib->cycphs);
		ercd = E_OK;
	}
	t_unlock_cpu();
