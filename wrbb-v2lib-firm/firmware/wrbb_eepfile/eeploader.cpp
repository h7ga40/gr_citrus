...
#else
			USB_Serial->write((unsigned char)fp.read());
#endif
		}
		else{
#ifdef GRCITRUS
	...
	while(true){
		...
		if (CommandData[0] == 'Z'){
			...
			if((bool)root) for (;;) {
				...
				USB_Serial->print(" ");
				USB_Serial->print(fp.name());
				USB_Serial->print(" ");
				USB_Serial->print(fp.size());
				USB_Serial->println(" byte");
